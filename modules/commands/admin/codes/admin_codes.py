...
@dp.message_handler(commands=["new_admin_code"], function=lambda message: main_admin(message))
async def new_admin_code(message):
    ...
    session.commit()
    code_db = session.query(Code).filter(Code.code == code, Code.type == 1).first()
    await bot.send_message(ADMIN_ID, f"Новый админ-код с id: {code_db.id}. Придумайте ему название: {code_db.id}. Название")
    admin_codes_logger.info(f"Новый админ-код с id: {code_db.id}")
    change_state(message, NEW_ADMIN_CODE_TITLE_STATE)


@dp.message_handler(commands=["edit_admin_title"], function=lambda m: main_admin(m))
async def edit_admin_title(message):
    await new_code_title(message, " ".join(message.text.split()[1:]), 'admininvite')


@dp.message_handler(content_types=["text"], function=lambda m: get_state(m) == NEW_ADMIN_CODE_TITLE_STATE)
async def new_admin_code_title(message):
    await new_code_title(message, message.text, 'admininvite')


@dp.message_handler(commands=["all_admin_codes"], function=lambda message: main_admin(message))
