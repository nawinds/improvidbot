...
@bot.message_handler(commands=["my_referal"])
def get_referal_link(message):
    bot.send_message(message.from_user.id, f"t.me/{BOT_USERNAME}?start=pr-{message.from_user.id}")
...
def top(message):
    ...
    data = [f"{user + 1}. <b>{top_users[user].score}</b> "
            f"<a href=\"t.me/{bot.get_chat(top_users[user].tg_id).username}\">"
            f"{bot.get_chat(top_users[user].tg_id).first_name} "
            f"{bot.get_chat(top_users[user].tg_id).last_name if bot.get_chat(top_users[user].tg_id).last_name else ''}"
            f"</a>" if bot.get_chat(top_users[user].tg_id).username else
...
def my_videos(message):
    ...
    bot.send_message(message.from_user.id, f"Ваши загруженные видео:\n\n"
