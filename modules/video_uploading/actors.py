...
@bot.message_handler(func=lambda message: get_state(message) == NEW_VIDEO_ACTORS_STATE)
def new_video_actors(message):
    actor = message.text
    if actor == "Всё":
        ...
        bot.send_message(message.from_user.id, "Отлично! Выбранные актёры: " + ", ".join(actors),
                         reply_markup=markup)
        markup = ReplyKeyboardMarkup()
        markup.add("Всё")
        bot.send_message(message.from_user.id, "<b>Теперь нужно добавить к видео теги</b>. Здесь хорошо будет "
        ...
        bot.send_message(message.from_user.id, f"Пожалуйста, напишите только имя и фамилию двумя словами. "
        ...
        bot.send_message(message.from_user.id, f"Слишком длинное имя (более 30 символов). "
    ...
    if actor != "Всё":
        ...
        if tag_db:
            ...
            session.add(Tag(tag=tag, added_id=message.from_user.id, video_id=video.id,
                            coef=4 * 2))
            session.commit()
        bot.send_message(message.from_user.id, f"{actor}, отлично. Кто-то ещё?\n\n"
                                               f"<i><a href=\"https://telegra.ph/Kak-zagruzhat-video-v-bazu-bota-"
                                               "i-uvelichivat-ih-prosmotry-07-03\">"
                                               "Советы по оформлению ролика</a>\n"
                                               "/cancel для отмены загрузки и удаления ролика</i>",
