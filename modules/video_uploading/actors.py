...
async def new_video_actors(message):
    ...
    if actor == "Всё":
        ...
        await bot.send_message(message.from_user.id,
                               "<b>Теперь нужно добавить к видео теги</b>. Здесь хорошо будет "
        ...
        await bot.send_message(message.from_user.id, f"Пожалуйста, напишите только имя и фамилию двумя словами. "
                                                     ...
                                                     f"i-uvelichivat-ih-prosmotry-07-03\">"
                                                     f"Советы по оформлению ролика</a>\n"
                                                     f"/cancel для отмены загрузки и удаления ролика</i>",
                               parse_mode="html", disable_web_page_preview=True)
        logger.info("Некорректное кол-во слов в имени актера")
        return
    elif any([i not in WHITE_SYMBOLS for i in actor]):
        await bot.send_message(message.from_user.id,
                               f"Пожалуйста, напишите только имя и фамилию двумя словами. "
        ...
        await bot.send_message(message.from_user.id,
                               f"Слишком длинное имя (более 30 символов). "
